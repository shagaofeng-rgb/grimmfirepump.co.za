...
function validSign(sign: string) { return equal(sign, process.env.BLOG_WEBHOOK_SECRET ?? ""); }
export async function GET(request: NextRequest) { return validSign(safeText(request.nextUrl.searchParams.get("sign"), 512)) ? reply(1, "接口验证成功") : reply(0, "秘钥错误", 401); }
export async function POST(request: NextRequest) {
  ...
  const sign = safeText(input.sign, 512); if (!validSign(sign)) return reply(0, "秘钥错误", 401);
  ...
  if (!title && !content) return reply(1, "接口验证成功");
