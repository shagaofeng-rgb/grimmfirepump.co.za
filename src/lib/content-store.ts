...
type NewsRow = Omit<NewsArticle, "content" | "relatedProductIds" | "publishedAt"> & { content: unknown; related_product_ids: unknown; published_at: string; source_name: string | null; source_url: string | null };
...
export async function getPublishedNews(): Promise<NewsArticle[]> {
  ...
  await ensureSeedCatalog();
  const rows = await getDatabase()`SELECT id, slug, title, excerpt, content, category, status, published_at, source_name, source_url, related_product_ids FROM news_articles WHERE status = 'published' AND deleted_at IS NULL ORDER BY published_at DESC`;
...
export async function getNewsArticle(slug: string): Promise<NewsArticle | undefined> {
  ...
  await ensureSeedCatalog();
  const rows = await getDatabase()`SELECT id, slug, title, excerpt, content, category, status, published_at, source_name, source_url, related_product_ids FROM news_articles WHERE slug = ${slug} AND status = 'published' AND deleted_at IS NULL LIMIT 1`;
