...
function score(title: string, summary: string) { const words = `${title} ${summary}`.toLowerCase(); const terms = ["fire", "pump", "water", "dewater", "supply", "industrial", "safety", "standard", "regulation", "infrastructure"]; return Math.min(100, terms.reduce((total, term) => total + (words.includes(term) ? 12 : 0), 0)); }
...
export async function runNewsIngest() {
  ...
  try {
    ...
    for (const source of sources) for (const item of await items(source.rss_url)) {
      discovered++; const title = plain(String(item.title ?? "")); const sourceUrl = canonical(String(item.link ?? "")); const date = new Date(String(item.pubDate ?? item.published ?? item.updated ?? "")); const summary = plain(String(item.description ?? item.summary ?? "")).slice(0, 1500); const age = Date.now() - date.valueOf(); const candidateScore = score(title, summary);
...
export async function runNewsPublish() {
  ...
  try {
    ...
    const body = await detail.text(); const verified = list.ok && detail.ok && sitemap.ok && rss.ok && body.includes(title);
