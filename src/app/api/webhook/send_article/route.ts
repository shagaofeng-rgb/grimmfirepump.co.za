...
export async function GET(request: NextRequest) {
  return validSign(safeText(request.nextUrl.searchParams.get("sign"), 512)) ? reply(1, "验证成功") : reply(0, "秘钥错误", 401);
}
...
export async function POST(request: NextRequest) {
  ...
  if (!validSign(sign)) return reply(0, "秘钥错误", 401);
  ...
  try {
    ...
    return reply(1, result.duplicate ? "发布成功（重复请求已安全忽略）" : "发布成功");
  } catch {
